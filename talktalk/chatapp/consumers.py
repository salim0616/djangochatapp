...
from channels.exceptions import DenyConnection, RequestAborted, StopConsumer
...
class MsgConsumer(WebsocketConsumer):
    def get_messages(self):
        q = Q(
            Q(sender=self.scope["sender"], receiver=self.scope["receiver"])
            | Q(sender=self.scope["receiver"], receiver=self.scope["sender"])
        )
        messages = MessageSerializer(
            Message.objects.filter(q).order_by("-timestamp"), many=True
        ).data
        return messages
    ...
    def auth_check(self, scope):
        ...
        if (b"authorization" in headers) and (b"receiver" in headers):
            ...
            receiver_qry = User.objects.filter(
                Q(is_online=True) & Q(token_expiry__gte=timezone.now()),
    ...
    def connect(self):
        try:
            ...
            self.room_name = "".join(
                sorted([self.scope["sender"].username, self.scope["receiver"].username])
            )
